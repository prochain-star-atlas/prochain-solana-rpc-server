use {
    ...
    }, futures::{sink::SinkExt, StreamExt}, solana_account::Account, solana_pubkey::Pubkey, solana_signature::Signature, std::{
        collections::{HashMap, HashSet},
        sync::Arc,
        time::Duration,
    }, tokio::sync::{mpsc::Sender, RwLock}, tokio_util::sync::CancellationToken, yellowstone_grpc_client::GeyserGrpcClient, yellowstone_grpc_proto::{
...
#[derive(Debug)]
pub struct ProchainYellowstoneGrpcGeyserClient {
    pub endpoint: String,
...
impl ProchainYellowstoneGrpcGeyserClient {
    pub const fn new(
        endpoint: String,
        ...
        ProchainYellowstoneGrpcGeyserClient {
            endpoint,
            x_token,
            commitment,
            account_filters,
...
#[async_trait]
impl Datasource for ProchainYellowstoneGrpcGeyserClient {
    async fn consume(
        &self,
        id: DatasourceId,
        ...
        let cloned_sender_used = sender.clone();
                                    ...
                                    match message {
                                        Ok(msg) => match msg.update_oneof {
                                            ...
                                            Some(UpdateOneof::Transaction(transaction_update)) => {
                                                send_subscribe_update_transaction_info(transaction_update.transaction, &metrics, &cloned_sender_used, id_for_loop.clone(), transaction_update.slot, None).await
                                            }
                                            Some(UpdateOneof::Block(block_update)) => {
                                                ...
                                                for transaction_update in block_update.transactions {
                                                    if retain_block_failed_transactions || transaction_update.meta.as_ref().map(|meta| meta.err.is_none()).unwrap_or(false) {
                                                        send_subscribe_update_transaction_info(Some(transaction_update), &metrics, &cloned_sender_used, id_for_loop.clone(), block_update.slot, block_time).await
                                                    }
                                                }
