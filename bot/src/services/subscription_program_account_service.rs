...
#[dynamic] 
static LIST_PROGRAM_ACCOUNT_SUBSCRIPTION: Mutex<DashMap<String, Vec<String>>> = Mutex::new(DashMap::new());
...
pub struct SubscriptionProgramAccountService {
}

impl SubscriptionProgramAccountService {

    pub async fn init_sub(sub_name: String) {

        let arc_state = solana_state::get_solana_state();
        let arc_state_cloned = arc_state.clone();

        let programs = get_mutex_program_sub(sub_name.clone());   
        for prog in programs.iter() {
            log::info!("starting processing program: {}", prog);

            let res = arc_state.get_sol_client().get_program_accounts(&Pubkey::try_from(prog.as_str()).unwrap()).await;
            ...
            if res.is_ok() {
                ...
                response.iter().for_each(|acc| {
                    ...
                    arc_state_cloned.add_account_info(acc.0, tt);
    ...
    pub async fn restart() {
        ...
        let jh = SubscriptionProgramAccountService::start_monitor().await;
    ...
    pub async fn start_monitor() -> Result<CancellationToken, anyhow::Error> {
        ...
        *data_count = data_count.add(1);
        ...
        account_filters.insert(
            "spas_".to_string() + data_count.to_string().as_str(),
