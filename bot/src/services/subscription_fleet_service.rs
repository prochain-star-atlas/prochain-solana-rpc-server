...
#[dynamic] 
static LIST_FLEET_SUBSCRIPTION: Mutex<DashMap<String, Vec<String>>> = Mutex::new(DashMap::new());

pub fn set_mutex_fleet_sub(sub_name: String, lst_vec: Vec<String>) {
    LIST_FLEET_SUBSCRIPTION.lock().insert(sub_name, lst_vec);
}

pub fn get_mutex_fleet_sub(sub_name: String) -> Vec<String> {
...
pub fn reset_all_list_sub() {
    LIST_FLEET_SUBSCRIPTION.lock().clear();
}

#[derive(Debug, Clone, Default)]
pub struct SubscriptionFleetService {
}

impl SubscriptionFleetService {

    pub async fn restart() {

        let jh = SubscriptionFleetService::start_monitor().await;
    ...
    pub async fn start_monitor() -> Result<CancellationToken, anyhow::Error> {
        ...
        let list_add: Vec<String> = get_mutex_fleet_sub("sage".to_string()).into_iter().map(|f| { f }).collect();
        ...
        *data_count = data_count.add(1);
        ...
        log::info!("Start subscription for fleet ...");
